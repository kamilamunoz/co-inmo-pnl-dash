...
RAW_PATH = REPO_ROOT / "data" / "raw_apartment_inmo_co.parquet"
...
def _long_to_nested(long_df: pd.DataFrame) -> dict:
    ...
    preservar precisión (%; en montos con 2 decimales es suficiente).
    """
    out: dict = {}
...
def main() -> None:
    ...
    payload = {
        "meta": {
            ...
            "tabla_fuente": "clients-domain-data-master.finance_wh_bi.finance_apartment_tracker_inmobiliaria_co",
            "cohorte": "fecha_factura (facturación NetSuite Inmo CO)",
            "currency": "COP",
            "unidad": "unidades absolutas (el frontend divide por 1_000_000 para mostrar en millones COP)",
            ...
            "rango_fechas": {
                "min": pd.to_datetime(df["fecha_factura"]).min().strftime("%Y-%m-%d"),
                "max": pd.to_datetime(df["fecha_factura"]).max().strftime("%Y-%m-%d"),
            },
        },
