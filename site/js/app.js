// co-inmo-pnl-dash — frontend
// Kamila mantiene sola. Vanilla JS, sin frameworks.

const PASSWORD = 'p&L_Inm0*C0l*C12d4d';
const STORAGE_KEY = 'co-inmo-pnl-auth';

// Webhook de Google Chat para reportar NIDs con signo raro a Jeff.
// Reutiliza el webhook común; los mensajes se etiquetan [INMO CO].
...
const REPORTER_NAME = 'Kamila (dashboard CO P&L Inmobiliaria)';
...
function renderHeader() {
  ...
  document.getElementById('contextLabel').textContent =
    `Inmobiliaria · ${m.filas_incluidas.toLocaleString('es-CO')} NIDs facturados`;
...
function fmt(v, opts = {}) {
  ...
  // Los valores en el JSON están en COP absolutos; dividimos por 1_000_000 para
  // mostrar en millones COP (consistente con co-city-pnl-dash MM).
  if (v === null || v === undefined) return '—';
  if (opts.isCount) return Math.round(v).toLocaleString('es-CO');
  if (opts.isPct) {
    if (!isFinite(v)) return '—';
    return (v * 100).toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '%';
  }
  if (opts.isTicket) {
    // Ticket promedio en millones COP, con 1 decimal
    return (v / 1_000_000).toLocaleString('es-CO', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
  }
...
function openDrill(row, mes) {
  ...
  document.getElementById('drillTotal').innerHTML =
    `<span class="amount ${totalCls}">${fmtAbs(total)}</span><span class="lbl">COP · ${items.length.toLocaleString('es-CO')} NIDs</span>`;
...
function renderCmp() {
  ...
  document.getElementById('cmpContext').textContent =
    `${state.cmpVista === 'acc' ? 'ACC' : 'Sintético'} · ${cmpMesLabel(meses)} · ${state.cmpMetrica === 'abs' ? 'COP 000\'s (con % del Revenue debajo)' : state.cmpMetrica === 'pct' ? '% del Revenue de la región' : 'COP MM por NID facturado'}`;
...
function openCmpDrill(row, region, meses) {
  ...
  const totalCls = row.sign === 'cost' ? 'cost' : (row.sign === 'income' ? 'income' : '');
  document.getElementById('drillTotal').innerHTML =
    `<span class="amount ${totalCls}">${fmtAbs(total)}</span><span class="lbl">COP · ${items.length.toLocaleString('es-CO')} NIDs (agregado)</span>`;
...
function buildReportMessage(ctx) {
  ...
  const text =
`🚩 *NIDs con signo raro · [INMO CO] P&L*
